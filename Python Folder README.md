...
To make it easier to produce new escape rooms without rebuilding notebooks from scratch, I built a browser-based generator. It lives in the `escape-room-generator` folder in this repository.

### What It Does

Teachers configure grade (3–11), skill level (Beginner or Intermediate), format, and topics. The tool generates ready-to-use Google Colab notebooks — one file for Solo format, or one file per student for Team Chain format.

No installation required. Open the HTML file in any browser.

### Two Modes

**Library Mode** draws from a hand-written bank of 3 challenge variants per topic, group, and skill combination. A random variant is selected on each generate. No account needed, works offline, assert checks are hand-verified.

**AI Mode** uses Claude to write a unique scenario, starter code, and assert checks every time. A custom theme can be specified. Requires an Anthropic API key.

> ⚠️ AI-generated assert checks can occasionally contain logic errors. Always run through an AI-generated notebook yourself before using it with students.

### Formats

**Solo** — one notebook, one student works through all locks in sequence.

**Team Chain** — one notebook per student. Each student solves one lock and passes their fragment to the next student, who uses it as an entry code to unlock their own notebook. The last student assembles the final password.

### Topics Covered

13 concepts aligned to the course curriculum, from Variables & Print through to the Random Library. Topics incompatible with the selected grade and skill are greyed out automatically.

See the `escape-room-generator` folder for full documentation and the generator file itself.
